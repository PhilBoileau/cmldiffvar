...
melt_tte_data_fun <- function(
  ...
  obs_ls <- lapply(
    ...
    function(obs_idx) {
      ...
      # label each entry by the time, create the longitudinal indicators,
      # and add an id
      long_obs_tbl <- long_obs_tbl |>
        dplyr::mutate(
          long_time = seq_len(time_cutoff),
          R = dplyr::if_else(long_time == .data[[outcome_var_name]] &
                               .data[[censoring_var_name]] == 1, 1, 0),
          L = dplyr::if_else(long_time == .data[[outcome_var_name]] &
                               .data[[censoring_var_name]] == 0, 1, 0),
          I = dplyr::if_else(long_time < .data[[outcome_var_name]], 1, 0),
          J = dplyr::case_when(
            I == 1 ~ 1,
            long_time == .data[[outcome_var_name]] &
      ...
      long_obs_tbl |>
        dplyr::select(
          dplyr::all_of(
            c("cmldiffvar_id", baseline_var_names, treatment_var_name, "R", "L",
              "I", "J")
          )
        )
    }
