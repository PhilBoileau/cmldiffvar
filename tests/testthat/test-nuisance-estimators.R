...
test_that("conditional expected outcome^2 estimator glm.gamma wrapper
          predicts > 0", {
...
test_that("glm.gamma wrapper adapts to multiple confounders", {
  ...
  cond_exp_sq_outcome_correct_fit <- estimate_cond_exp_sq_outcome_fun(
    sample_tbl,
    confounder_var_names = c("W1", "W2", "W3"),
    treatment_var_name = "treatment",
    outcome_var_name = "outcome",
    cond_exp_sq_outcome_library = c("SL.glm.gamma"),
    num_nuisance_sl_folds = 5
  ...
  real_colnames <- c(
    "(Intercept)",
    "treatment", "W1", "W2", "W3",
    "I(W1 * treatment)", "I(W2 * treatment)", "I(W3 * treatment)",
    "I(W1 * W2)", "I(W1 * W3)", "I(W2 * W3)",
